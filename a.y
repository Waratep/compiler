...
assignment: identifier '=' exp  { sym[var_counter].name = $1; sym[var_counter].value = $3;  var_counter++; }
	;
...
term: number              {$$ = $1;}
	| identifier						{$$ = val_of_iden($1);} 
	;

%%                   


int val_of_iden(char * str){
	for(int i = 0; i < 255 ; i++){
...
int main (int argc , char ** argv) {
	...
	if(argc < 2){
		printf("Sawaddeeja. ni keu \" pasa karaoke\". version 1.0.0 \n");
		return yyparse();
	}else{
        yyin = fopen(argv[1], "r");
        ...
        if(var_counter != 0){
            ...
            fprintf(yyout, "; Sawaddeeja. ni keu \" pasa karaoke\". version 1.0.0 \n\n"); 
            print_data();
